# SVT Test
# SVT version 25.4.b3
# Test saved 2025-08-01_1145
# Form factor: SV5C_CPTX_CPRX
# PY3
# Checksum: 685840be76b120f45f1949ad98f333df
...
calOptions.addField('dataRate', descrip='''calibration dataRate''', attrType=float, iespInstanceName='any', defaultVal=4500.0, displayOrder=(0, 1.0))
calOptions.addField('depth', descrip='''''', attrType=float, iespInstanceName='any', defaultVal=100000000.0, displayOrder=(0, 2.0))
calOptions.addField('lanes', descrip='''''', attrType=list, iespInstanceName='any', attrSubType=int, defaultVal=[1, 2, 3, 4], displayOrder=(0, 3.0))
calOptions.addField('wires', descrip='''''', attrType=list, iespInstanceName='any', attrSubType=str, defaultVal=['wireAB', 'wireBC', 'wireCA'], displayOrder=(0, 4.0))
calOptions.addField('wireSubChannels', descrip='''''', attrType=list, iespInstanceName='any', attrSubType=str, defaultVal=['0', '1'], displayOrder=(0, 5.0))
calOptions.addField('defaultSyncThreshold', descrip='''''', attrType=float, iespInstanceName='any', defaultVal=45.0, displayOrder=(0, 6.0))
calOptions.addField('maxSyncThreshold', descrip='''''', attrType=float, iespInstanceName='any', defaultVal=300.0, displayOrder=(0, 7.0))
calOptions.addField('serialNumber', descrip='''''', attrType=str, iespInstanceName='any', defaultVal='89-xxxx', displayOrder=(0, 8.0))
...
''',
False)
calOptions.dataRate = 4000.0
calOptions.depth = 100000000.0
calOptions.lanes = [1, 2, 3, 4]
calOptions.wires = ['wireAB', 'wireBC', 'wireCA']
calOptions.wireSubChannels = ['0', '1']
calOptions.defaultSyncThreshold = 45.0
calOptions.maxSyncThreshold = 300.0
calOptions.serialNumber = '89-xxxx'
...
currThreshold = calOptions.defaultSyncThreshold
#Eye width must measure at least 85% of UI
minEyeWidth_ps = (1000000 / calOptions.dataRate) * 0.85
print(minEyeWidth_ps)
eyeWidth = 0

while((eyeWidth < minEyeWidth_ps) and (currThreshold < calOptions.maxSyncThreshold)):
...
runFirstBertscan.code = r'''laneList1.lanes = calOptions.lanes[0]
laneList1.setup()
lane = calOptions.lanes[0]
bertScan1.wires = calOptions.wires[0]
wire = calOptions.wires[0]
wireSubChannel = calOptions.wireSubChannels[0]
phase = 'even'
...
with open(filePath, "w") as outFile:
    ...
    outFile.write("serial number : " + calOptions.serialNumber + "\n")
...
uiInPs = 1/calOptions.dataRate*1000000
setDataRate(calOptions.dataRate)
...
resultDict = dict()
phases = ['even','odd']
for lane in calOptions.lanes :
    resultDict[lane] = dict()
    for wire in calOptions.wires:
            resultDict[lane][wire] = dict()
            for wireSubChannel in calOptions.wireSubChannels:
...
depth = calOptions.depth
lanes = calOptions.lanes
wires = calOptions.wires
wireSubChannels = calOptions.wireSubChannels
...
for lane in calOptions.lanes:
    referenceResultEven = resultDict[lane][calOptions.wires[0]][calOptions.wireSubChannels[0]][phases[0]]
    ...
    referenceResultOdd = resultDict[lane][calOptions.wires[0]][calOptions.wireSubChannels[0]][phases[1]]
    ...
    for wire in calOptions.wires:
        for wireSubChannel in calOptions.wireSubChannels:
            ...
            if (falling2FallingSkew > 0.5*uiInPs) :
                desiredSkew = falling2RisingSkew
                print("Using the falling edge of %s against the rising edge of the reference wire..."%wire)
            elif (falling2FallingSkew < -0.5*uiInPs) :
                desiredSkew = rising2FallingSkew
                print("Using the rising edge of %s against the rising edge of the reference wire..."%wire)
            else :
                desiredSkew = falling2FallingSkew
            finalSkewList.append(int( (desiredSkew)*1000) )

print("\n")
print("\n")
print("Data rate is %d Msps..." % calOptions.dataRate)
print("Final skew list for cal file is:")
print(finalSkewList)

if len(finalSkewList) == 24:
    writeTransferFunctions(finalSkewList)
    print("####################")
    print("####################")
    print("Re-label result folder as unit serial number: %s"%calOptions.serialNumber)
else:
