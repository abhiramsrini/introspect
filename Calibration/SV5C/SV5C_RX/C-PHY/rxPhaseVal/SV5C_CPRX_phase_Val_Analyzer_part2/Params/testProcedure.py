# SVT Test
# SVT version 25.1.1
# Test saved 2025-07-07_1734
# Form factor: SV5C_4L8G_MIPI_CPHY_ANALYZER
# PY3
# Checksum: ded7f3cac11d8d8fd3be7312234def82
...
runIndividualEyeScans = _create('runIndividualEyeScans', 'SvtFunction', iespName=None)
...
runIndividualEyeScans.args = ''
runIndividualEyeScans.code = r'''# --------------------------------------------------------------------
printMsg('Doing eye scans at %.1fMsps: Each wire seperately...' % calParams.dataRate, 'magenta', None, True)
...
for preBert in range(24):
    ...
    for trial in range(numberOfMaxTrials):
        ...
        print(eyeCenterY)
        if eyeCenterY == None:
            warningMsg("Found a failing condition on %s%d_%d... eyeCenterY=None! Repeat..." % (calParams.wires[wire], lane, preBert%2))
            # set the clock again
            SetupFunction(calParams.dataRate)
        else:
            break
...
# --------------------------------------------------------------------
'''
runIndividualEyeScans.wantAllVarsGlobal = False
