# SVT Test
# SVT version 24.1.0
# Test saved 2024-03-08_1349
# Form factor: SV3C_4L3G_MIPI_CPHY_GENERATOR
# PY3
# Checksum: 81db9d352453f969cf3fce3edac2d840
...
osci.WriteString("BEEP", 1)
osci.WriteString('MESSAGE "Connected from Python"', 1)
...
osci.WriteString("VBS 'app.Acquisition.C1.FindScale'", 1)
osci.WriteString("VBS 'app.Acquisition.C2.FindScale'", 1)
osci.WriteString("VBS 'app.Acquisition.C3.FindScale'", 1)

# Clear the display (acquisition memory, traces)
osci.WriteString("VBS 'app.Acquisition.ClearSweeps'", 1)

# Measure average voltage of channel 1 to set trigger level
# Set average voltage measurement on Channel 1 (Parameter 1)
...
avg_voltage_str = osci.WriteString("VBS? 'return=app.Measure.P1.Out.Result.Value'", 1)
currentValue = float(avg_voltage_str.strip())

# Set amplitude measurement on Channel 1 (Parameter 2)
...
amplitude_str = osci.WriteString("VBS? 'return=app.Measure.P2.Out.Result.Value'", 1)
currentAmp = float(amplitude_str.strip())

print("Average Voltage on C1:", currentValue)
print("Amplitude on C1:", currentAmp)

triggerValue = currentValue - 0.25*currentAmp

# Set trigger level to be just below the mid-point of the 3-level waveform
osci.WriteString('VBS \'app.Acquisition.Trigger.Edge.Source = "C1"\'', 1)
osci.WriteString(f'VBS \'app.Acquisition.Trigger.Edge.Level = {triggerValue}\'', 1)
sleepMillis(calOptions.scopeMeasurementDelay)
...
for channel in [1,2,3] :
    ...
    sleepMillis(calOptions.scopeMeasurementDelay)
    avg_voltage_str = osci.WriteString("VBS? 'return=app.Measure.P1.Out.Result.Value'", 1)
    varAverage = float(avg_voltage_str.strip())
    ...
    osci.WriteString("VBS 'app.Measure.P1.ParamEngine = \"Amplitude\"'", 1)
    sleepMillis(calOptions.scopeMeasurementDelay)
    amplitude_str = osci.WriteString("VBS? 'return=app.Measure.P2.Out.Result.Value'", 1)
    varAmp = float(amplitude_str.strip())
...
validationOptions.addField('scopeMeasurementDelay', descrip='''Amount of average accumulation time in milliseconds.''', attrType=float, iespInstanceName='any', defaultVal=3000.0, displayOrder=(0, 3.0))
...
validationOptions.addField('calDataRate', descrip='''Base data rate for performing voltage measurements''', attrType=float, iespInstanceName='any', defaultVal=500.0, displayOrder=(0, 5.0))
...
validationOptions.addField('commonModeValues', descrip='''Range of common-mode voltages to be verified''', attrType=list, iespInstanceName='any', attrSubType=float, defaultVal=[130.0, 390.0], displayOrder=(0, 7.0))
validationOptions.addField('amplitudeValues', descrip='''Range of differential voltages to be verified''', attrType=list, iespInstanceName='any', attrSubType=float, defaultVal=[40.0, 270.0, 290.0], displayOrder=(0, 8.0))
validationOptions.addField('connectedToTerminationBoard', descrip='''Set to true if the measurement is performed using a termination board with high-impedance probes. If the measurement is done directly on the oscilloscope 50 Ohm inputs, set to False. If set to False, a compensation value for common mode measurements is applied''', attrType=bool, iespInstanceName='any', defaultVal=False, displayOrder=(0, 9.0))
validationOptions.addField('absoluteErrorThreshold', descrip='''Pass/fail threshold for the absolute value of voltage error''', attrType=float, iespInstanceName='any', defaultVal=25.0, displayOrder=(0, 10.0))
validationOptions.addField('percentErrorThreshold', descrip='''Pass/fail threshold for the percentage of voltage error''', attrType=float, iespInstanceName='any', defaultVal=10.0, displayOrder=(0, 11.0))
validationOptions.addField('scopeConnectionTimeout', descrip='''Scope connection timeout.''', attrType=float, iespInstanceName='any', defaultVal=10000.0, displayOrder=(0, 12.0))
validationOptions.addField('minVersion', descrip='''Minimum Introspect ESP software version that is supported by this script.''', attrType=str, iespInstanceName='any', defaultVal='22.2.1', displayOrder=(0, 13.0))
...
''',
False)
validationOptions.serialNumber = 'CPTX17120013'
validationOptions.scopeIPAddress = 'TCPIP0::169.254.103.240::inst0::INSTR'
...
validationOptions.commonModeValues = [-80.0, 130.0, 250.0, 390.0]
validationOptions.amplitudeValues = [40.0, 150.0, 250.0, 380.0]
validationOptions.connectedToTerminationBoard = False
validationOptions.absoluteErrorThreshold = 25.0
