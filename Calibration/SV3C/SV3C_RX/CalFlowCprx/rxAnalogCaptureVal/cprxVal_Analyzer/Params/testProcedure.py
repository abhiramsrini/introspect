# SVT Test
# SVT version 22.4.0
# Test saved 2022-11-23_1147
# Form factor: SV3C_4L3G_MIPI_CPHY_ANALYZER2
# PY3
# Checksum: 0419bfd1182deea35d0e10061f3b0ff3
...
calOptions.addField('targetLevels', descrip='''List of Rx threshold voltage levels to be measured.''', attrType=list, iespInstanceName='any', attrSubType=float, defaultVal=[150.0, 200.0, 250.0, 300.0], displayOrder=(0, 3.0))
...
calOptions.targetLevels = [150.0, 200.0, 250.0, 300.0]
...
analogCapture1.endVoltage = 400.0
...
analogCapture1.startVoltage = -400.0
    ...
    for targetLevel in sorted(calOptions.targetLevels) :
        ...
        for lane in sorted(laneList1.lanes) :
            for wire in ['wireAB', 'wireBC', 'wireCA'] :
                ...
                if ((averageVoltagePerLaneHigh[lane][wire] > targetLevel + targetLevel*0.20 or averageVoltagePerLaneHigh[lane][wire] < targetLevel - targetLevel*0.20  ) or math.isnan(averageVoltagePerLaneHigh[lane][wire])):
                    failFlag = 1
                    warningMsg("Bad Value on Lane %d wire %s..." % (lane, wire))
                    print(averageVoltagePerLaneHigh[lane][wire])
                    print(targetLevel)
                if ((averageVoltagePerLaneLow[lane][wire] > -targetLevel + targetLevel*0.20 or averageVoltagePerLaneLow[lane][wire] < -targetLevel - targetLevel*0.20  ) or math.isnan(averageVoltagePerLaneLow[lane][wire])):
                    failFlag = 1
                    warningMsg("Bad Value on Lane %d wire %s...." % (lane, wire))
                    print(averageVoltagePerLaneLow[lane][wire])
                    print(targetLevel)
